...
Eric: My main takeaway from this assignment is that statistical computing could be utilized in professional fields to predict trends and allocate resources accordingly. Extending beyond the requirements and results of this project, I think the American Bar Association could rely on the models we created to decide which states need more divorce lawyers and collaborate accordingly with state governments on the matter. Although we were limited to data collected on lower-income individuals and volunteer attorneys, I suspect the trends we discovered reflect other income brackets as well. However, by far the biggest limitation with exploring our research question was the lack of data from some states, and the seemingly uneven availability of data among other states. For instance, the client data set we worked with only recorded one client from Idaho, essentially nullifying it entirely. The fact that California only recorded twice as many questions as Wyoming, a state about one-eightieth the population, is also concerning. 

What I also grasped from completing this project is the importance of formulating a goal in mind early when working with messy data. Had my group blindly used the provided csv files to create our models and visualizations without first organizing the data, we would have spent much longer trying to pluck the results for something insightful, not to mention the loss of accuracy that may result from NULL values and outliers. With such a large quantity of data at our disposal, it becomes more important that we come up with specific questions we want to investigate early on and filter the data accordingly. As a student currently taking a GIS course, I find this idea to be invaluable in both fields, as I have learned that most online data provides much more than what is necessary.
...
* Finish Confidence interval and odds ratio interpretation, please readjust when needed,
* Ask Prince whether our odds ratio and confidence interval interpretation is valid,
* ask for further suggestions if there are any
