...
```{r eval= TRUE, include=TRUE, echo=FALSE}
model2 <- glm(factor(Subcategory) ~ factor(MaritalStatus) + SavingsBalance ,
...
# Model 4:
```{r eval= TRUE, include=TRUE, echo=FALSE}
model3 <- glm(Subcategory ~ AllowedIncome, data = data_overall,
              family = 'binomial')
summary(model3)
```
...
- Some assumptions to consider for our model
  ...
  - no sever multicollinearity aamong explanatory variables, no highly correlated
    variables
  - no extreme outliers
