...
Another crucial lesson we learned from this project is the significance of establishing a clear objective when dealing with unorganized data. If our team had blindly utilized the given csv files to develop models and visualizations without initially structuring the data, we would have encountered substantial delays in extracting meaningful insights. Additionally, we would have faced the risk of decreased accuracy due to the presence of NULL values and outliers. Given the extensive volume of data at our disposal, it becomes even more crucial to formulate specific research questions early on and filter the data accordingly. We consider this concept invaluable across various research domains, as the majority of datasets contain far more information than what is necessary for drawing significant conclusions.
...
Like any dataset and research inquiry, there are constraints that can impact the overall predictive capability of a research model. In the case of the dataset obtained from the American Bar Association pro bono service, we discovered that a select few states, precisely ten, lacked the provision of clients' demographic information. Consequently, during the data cleansing process where we merged and joined the dataframes using the unique identifier for each client, we were unable to obtain any client data for 20% of the United States. This poses a challenge as we cannot predict the legal queries of clients in those specific ten states based on their demographic background. Nevertheless, since we have excluded those states from our dataset, we can confidently affirm that we possess significant predictive power for clients residing in states that did provide client demographics.
