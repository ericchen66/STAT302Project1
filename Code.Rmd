...
# Data cleaning, select the top 5 Marital status
...
data_noOutlier <- data_noOutlier %>%
  filter(data_noOutlier$AnnualIncome > 0)

data_noOutlier <- subset(data_noOutlier, MaritalStatus %in% c("Single", "Married / remarried", "Divorced or Widowed", "Separated","I'd rather not answer"))
...
#   Box plot of median annual income level of people of top 5 marital status
#   Faceted by subcategory 
PlotAnnual <- ggplot(data = data_noOutlier, aes(x = AnnualIncome , y = MaritalStatus, fill 
                                  = MaritalStatus)) +
  geom_boxplot() +
...
#   Box plot of median checking balance level of people of top 5 marital status
#   Faceted by subcategory 
PlotAllowed <- ggplot(data = data_noOutlier, aes(x = AllowedIncome , y = MaritalStatus, fill 
                                  = MaritalStatus)) +
  geom_boxplot() +
...
#   Box plot of median savings balance level of people of top 5 marital status
#   Faceted by subcategory 
PlotSavings <- ggplot(data = data_noOutlier, aes(x = SavingsBalance , y = MaritalStatus, fill 
                                  = MaritalStatus)) +
  geom_boxplot() +
...
model_data <- subset(model_data, MaritalStatus %in% c("Single", "Married / remarried", "Divorced or Widowed", "Separated"))



# All independent variables used (in reference to Top 3 States and Single)
...
```{r eval= TRUE, include=FALSE, echo=FALSE}
library(MASS)
intercept_only <- glm(Subcategory ~ 1, data = model_data)
# Model 2: Using stepwise variable selection
model2 <- forward <- step(intercept_only, direction='both', scope=formula(model1), trace=0)
...
x <- data.matrix(model_data[, c("Age", "NumberInHousehold", "MaritalStatus", "AnnualIncome", "SavingsBalance", "CheckingBalance", "AllowedIncome", "GroupedStateAbbr")])
...
model3 <- glm(Subcategory ~ AnnualIncome + SavingsBalance + CheckingBalance, data = model_data, 
              family = 'binomial')
```
...
cat("Models Formulas:\n\n")
cat("Model 1: Subcategory ~ Age + NumberInHousehold + MaritalStatus + AnnualIncome + SavingsBalance + CheckingBalance + AllowedIncome + GroupedStateAbbr\n")
...
```{r eval= TRUE, include=FALSE, echo=FALSE}
# Model 1:All independent variables used 
model1 <- glm(Subcategory ~ . - AllowedIncome, 
...
```{r eval= TRUE, include=TRUE, echo=FALSE}
# Accuracy Table
acc_table <- table(factor(model_data$Subcategory), predict(model1)>0)
acc_table
# % Accuracy
